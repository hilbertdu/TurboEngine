...
namespace Math
{
	...
	// orthonormal set.
	template <typename Real>
	Real ComputeOrthogonalComplement(int numInputs, TVector2<Real>* v, bool robust = false);
	...
	template <typename Real>
	bool ComputeBarycentrics(TVector2<Real> const& p, TVector2<Real> const& v0,
		TVector2<Real> const& v1, TVector2<Real> const& v2, Real bary[3],
		Real epsilon = (Real)0);
	...
	public:
		...
		IntrinsicsVector2(int numVectors, TVector2<Real> const* v, Real inEpsilon);
	...
	TVector2<Real> Perp(TVector2<Real> const& v)
	{
		return TVector2<Real>{ v[1], -v[0] };
	}

	template <typename Real>
	TVector2<Real> UnitPerp(TVector2<Real> const& v, bool robust)
	{
		TVector2<Real> unitPerp{ v[1], -v[0] };
	...
	template <typename Real>
	Real ComputeOrthogonalComplement(int numInputs, TVector2<Real>* v, bool robust)
	{
		if (numInputs == 1)
		{
			v[1] = -Perp(v[0]);
		...
		Real epsilon)
	{
		...
		TVector2<Real> diff[3] = { v0 - v2, v1 - v2, p - v2 };
	...
	IntrinsicsVector2<Real>::IntrinsicsVector2(int numVectors,
		TVector2<Real> const* v, Real inEpsilon)
		:
		epsilon(inEpsilon),
		dimension(0),
		maxRange((Real)0),
