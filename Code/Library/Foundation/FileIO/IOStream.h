...
public:
	...
	inline bool Read(double & d) const	{ return (ReadBuffer(&d, sizeof(d)) == sizeof(d)); }
	...
	inline bool Write(const double & d) { return (WriteBuffer(&d, sizeof(d)) == sizeof(d)); }
	bool Write(const AString & string);
	template<class T> inline bool Write(const Array<T> & a);

	void ReadLine(AString & string, char delim = '\n') const;
