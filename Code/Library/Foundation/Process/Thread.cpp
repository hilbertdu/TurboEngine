...
#if defined(__WINDOWS__)
	static uint32 ThreadStartFunction(void * userData)
#else
...
/*static*/ Thread::ThreadHandle Thread::CreateThread(ThreadEntryFunction entryFunc,
	...
	void * userData)
{
	ThreadStartInfo& info = *(TNEW(ThreadStartInfo(entryFunc, userData, threadName)));
	MemoryBarrier();
...
// WaitForThread
//------------------------------------------------------------------------------
/*static*/ int Thread::WaitForThread(ThreadHandle handle, uint32 timeoutMS, bool & timedOut)
{
#if defined(__WINDOWS__)
