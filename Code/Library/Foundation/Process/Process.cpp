...
{
#if defined(__WINDOWS__)
	CASSERT(sizeof(m_ProcessInfo) == sizeof(PROCESS_INFORMATION));
#endif
}
...
// ReadAllData
//------------------------------------------------------------------------------
void Process::ReadAllData(AutoPtr<char> & outMem, uint32 * outMemSize, AutoPtr<char> & errMem, uint32 * errMemSize)
{
	// we'll capture into these growing buffers
	uint32 outSize = 0;
	uint32 errSize = 0;
	uint32 outBufferSize = 0;
	uint32 errBufferSize = 0;

	bool processExited = false;
	for (;;)
	{
		uint32 prevOutSize = outSize;
		uint32 prevErrSize = errSize;
...
//------------------------------------------------------------------------------
#if defined(__WINDOWS__)
void Process::Read(HANDLE handle, AutoPtr<char> & buffer, uint32 & sizeSoFar, uint32 & bufferSize)
{
	// anything available?
	DWORD bytesAvail(0);
	...
	if ((sizeSoFar + bytesAvail) > bufferSize)
	{
		...
		uint32 newBufferSize = Math::Max< uint32 >(sizeSoFar + bytesAvail, bufferSize + (16 * MEGABYTE));
...
//------------------------------------------------------------------------------
#if defined(__LINUX__) || defined(__APPLE__)
void Process::Read(int handle, AutoPtr<char> & buffer, uint32 & sizeSoFar, uint32 & bufferSize)
{
	// any data available?
	timeval timeout;
	timeout.tv_sec = 0;
	...
	uint32 spaceInBuffer = (bufferSize - sizeSoFar);
	if (spaceInBuffer == 0)
	{
		...
		// The caller has to take a copy to avoid the overhead if they want to hang onto the data
		// grow buffer in at least 16MB chunks, to prevent future reallocations
		uint32 newBufferSize = (sizeSoFar + (16 * MEGABYTE));
		char * newBuffer = (char *)ALLOC(newBufferSize + 1); // +1 so we can always add a null char
		if (buffer.Get())
		{
			// transfer and free old buffer
...
//------------------------------------------------------------------------------
#if defined(__WINDOWS__)
char * Process::ReadStdOut(uint32 * bytesRead)
{
...
//------------------------------------------------------------------------------
#if defined(__WINDOWS__)
char * Process::ReadStdErr(uint32 * bytesRead)
{
...
//------------------------------------------------------------------------------
#if defined(__WINDOWS__)
uint32 Process::ReadStdOut(char * outputBuffer, uint32 outputBufferSize)
{
...
//------------------------------------------------------------------------------
#if defined(__WINDOWS__)
uint32 Process::ReadStdErr(char * outputBuffer, uint32 outputBufferSize)
{
...
//------------------------------------------------------------------------------
#if defined(__WINDOWS__)
char * Process::Read(HANDLE handle, uint32 * bytesRead)
{
...
//------------------------------------------------------------------------------
#if defined(__WINDOWS__)
uint32 Process::Read(HANDLE handle, char * outputBuffer, uint32 outputBufferSize)
{
	// see if there's anything in the pipe
	DWORD bytesAvail;
	...
	uint32 bytesToRead = Math::Min<uint32>(outputBufferSize, bytesAvail);
...
// GetCurrentId
//------------------------------------------------------------------------------
/*static*/ uint32 Process::GetCurrentId()
{
#if defined(__WINDOWS__)
